    ...
    <MDBRow>
      ...
      >
        <MDBCard class="p-3 order-card">
          ...
          <p>
            <strong>Adresse de livraison:</strong> {{ order.Preference_Ship }}
          </p>
          <p>
            <strong>Date de livraison estimé au:</strong> {{ order.Ship_Date }}
          </p>
          <p>
            <strong>Prix de la commande:</strong> {{ order.Payment.Amount }} €
          </p>
          <p><strong>Produits:</strong></p>
          <ul class="product-list">
            <li
              v-for="product in order.Payment.Bills.split(',')"
              :key="product"
            >
              {{ product.trim() }}
            </li>
